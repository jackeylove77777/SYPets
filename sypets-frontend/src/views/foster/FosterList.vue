...
<!--      新增按钮-->
      <el-button type="primary" @click="dialogFormVisible=true" style="height: 40px;margin-left: 5px"  icon="el-icon-circle-plus-outline">发布寄养信息</el-button>
    </div>

<!--    内容主体-->
    <div class="mx-lg-2 mx-md-1 mt-4">
      <FosterItem v-for="item in list" :item="item" :key="item.id"></FosterItem>
    </div>
...
<!--点击新增按钮弹出的对话框-->
    <el-dialog title="发布寄养信息"  width="30%" :visible.sync="dialogFormVisible" class="rounded-4">
...
<script>
import FosterItem from "./FosterItem.vue";
export default {
  name: "FosterList",
  components:{
    FosterItem
  ...
  methods:{
    getList(){
      this.$http.get('/foster/all',{params:this.queryInfo}).then(res=>{
    ...
    addNew(){
      this.$refs['formRef'].validate(async (valid) => {
        if (valid) {
          const {data:res}=await this.$http.post('/foster',this.form)
